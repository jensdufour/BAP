...
Keep in mind that only the \acrlong{ltsc} version is supported. 
...
The migration is of interest for any kind of organization for the same reasons that were mentioned before. 
Additionally, SAP recommands the usage of the latest supported version for its products. 
Currently the latest supported version is Windows Server 2016 \acrshort{ltsc}.
...
\subsection{In-place upgrade of the SAP environment}
%TODO SAP In-place upgrade
...
\subsection{Side-by-side migration of the SAP environment}
%TODO SAP Side-by-side migration
As discussed in the previous section, an in-place upgrade is not always available from the \acrshort{os} of origin. 
As an alternative the side-by-side migration will also be performed. 
The starting point is the same as in the previous subsection.
