...
When migrating a server, it is of great importance that the environment around the specific server is entirely mapped. This to ensure that all the dependencies have been met. Afterwards, all of those should be verified. In this section, the migration from Windows Server 2016 to Windows Server 2019 will be done in an SAP environment. This to provide an example of the additional research that comes with migration of a server that is performing a critical task in the environment. 

\subsection{Technical specifications of SAP environment}
The proof of concept environment has been made using Microsoft Azure. It was partially based on the available resources on SAP \acrfull{cal}, adapted to the small size for this environment. A three-tier model was used as the basis of the environment. This means that it consists of the following: 
...
%TODO SAP Migration
In this subsection the full migration of the SAP application Server will be performed. This means adding a \acrshort{vm}, with the latest version of Windows Server, to the environment and installing the required packages to this server. It is important to verify that everything is supported using the SAP \acrfull{pam}.  Since the HANA database is currently only supported on Linux, this will not be migrated. With the database host not changing, the data does not need to be migrated.
...
% TODO Webinar avavin@microsoft.com 
