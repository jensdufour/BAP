...
This migration will first be performed using an in-place upgrade. 
Microsoft heavily focused on the improvement of this method. 
This should make it more resilient and safer to use. 
Afterwards, the process will be repeated using a traditional side-by-side migration. 
After this, the side-by-side migration of a typical SAP environment, as described by delaware, from Windows Server 2016 to Windows Server 2019 will be performed. 
Finally, the different versions of the base container images will be analysed. 
How these can lower virtual machine overhead and improve virtualization efficiency, will be reviewed. 
But first, the infrastructure that was used for most of this proof of concept, will be discussed.
...
\subsubsection{In-place upgrade to Windows Server 2019}
...
\subsubsection{Prerequisites}
The connection between the \acrshort{dc} and the client will be verified as was done in Subsection \ref{subsection:Inplace}.
...
In this section, the side-by-side migration from Windows Server 2016 to Windows Server 2019 will be done in a SAP environment. 
This to provide an example of the additional research that comes with the side-by-side migration of a server that is performing a critical task in a business environment. 
...
\subsection{Side-by-side migration of SAP}
%TODO SAP Migration
In this subsection, the side-by-side migration of the SAP application server will be performed. 
