...
After verifying the operation of the software solution, the in-place upgrade will be performed as described in Subsection \ref{sssec:In-place_upgrade}. There for the \acrfull{vhd} was downloaded from Azure.
...
For the third part to reduce the cut over time the backup and restore is used, followed by log shipping. When the last log is applied, the new system is ready to cut over. This way, the downtime is limited by the last log ship instead of the backup and restore of the database. 
...
For SAP, this was verified using the SAP \acrfull{pam}.
In the \acrfull{pam} all supported SAP solutions can be found.
