...
\end{lstlisting}
%TODO Graphs to benchmarks

 When looking at the result of the benchmarks of Server Core and Nano Server, it is clear that the performance of both versions is along the same lines. However, the overall reduction in size and the additional functions that have been added, as discussed in Chapter \ref{ch:stand-van-zaken}, make a strong case for the deployment of the latter version. Also, the arrival of the Windows base container image opens a whole array of new possibilities. The use case for each of those images will be discussed in the following subsection.
